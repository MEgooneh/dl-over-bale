#!/usr/bin/env python3
"""Receiver service entrypoint and reconstruction workflow."""
...
def handle_upload_done_payload(payload: dict[str, Any], state: dict[str, Any]) -> None:
    ...
    with state_lock:
        ...
        if mode == "chunked":
            job["payload_file_name"] = str(
                payload.get("payload_file_name") or job.get("payload_file_name") or ""
            ).strip()
...
def handle_control_message(client: httpx.Client, message: dict[str, Any], state: dict[str, Any]) -> None:
    ...
    if kind != "upload_done":
        return
    handle_upload_done_payload(payload, state)
