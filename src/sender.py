#!/usr/bin/env python3
"""Sender service entrypoint and transfer workflow."""
...
CHANNEL_UPDATES_CHAT_ID = str(os.environ.get("CHANNEL_UPDATES_CHAT_ID", CHANNEL_CHAT_ID)).strip()
...
UPLOAD_RETRIES = max(1, int(os.environ.get("UPLOAD_RETRIES", "8")))
...
ALLOWED_USER_IDS = {user_id.strip() for user_id in os.environ.get("ALLOWED_USER_IDS", "").split(",") if user_id.strip()}
...
def ensure_prerequisites() -> None:
    ...
    if not CHANNEL_UPDATES_CHAT_ID:
        raise RuntimeError("CHANNEL_CHAT_ID or CHANNEL_UPDATES_CHAT_ID is required.")
...
def init_db() -> None:
    with sqlite3.connect(DB_PATH) as conn:
        ...
        existing_columns = {str(row[1]) for row in conn.execute("PRAGMA table_info(requests)").fetchall()}
        ...
        uploaded_chunk_columns = {
            str(row[1]) for row in conn.execute("PRAGMA table_info(uploaded_chunk_messages)").fetchall()
        }
...
def retry_uploaded_requests(*, force: bool = False) -> None:
    for request_id, retry_count in list_uploaded_request_retries(force=force):
        ...
        try:
            ...
            log.info(
                "Re-sent upload completion for %s (%d/%d)", request_id, retry_count + 1, UPLOAD_CONFIRMATION_MAX_RETRIES
            )
        except Exception:
...
def delete_message(client: httpx.Client, chat_id: int | str, message_id: int) -> bool:
    result = api_call(client, "deleteMessage", json={"chat_id": str(chat_id), "message_id": int(message_id)}).get(
        "result"
    )
...
def format_cleanup_summary(cleaned_rows: list[dict[str, Any]]) -> str:
    ...
    return f"Chunk cleanup done.\n{request_count} requests | {chunk_count} chunks | {human_size(total_size)}"
...
def pick_downloaded_media_file(output_dir: Path) -> Path:
    candidates = [path for path in output_dir.rglob("*") if path.is_file() and not path.name.startswith(".part")]
    if not candidates:
...
def send_control_message(
    client: httpx.Client, chat_id: int | str, prefix: str, payload: dict[str, Any]
) -> dict[str, Any]:
...
def create_chunk_archive(stage_dir: Path, manifest: ChunkManifest, entry: dict[str, Any], raw_chunk_path: Path) -> Path:
    ...
    (chunk_stage_dir / "metadata.json").write_text(
...
def redownload_chunk(manifest: ChunkManifest, entry: dict[str, Any], destination: Path) -> None:
    ...
    if cache_path and cache_path.is_file():
        ...
        if total_bytes != size:
            ...
            raise RuntimeError(
                f"Recovered chunk size mismatch for chunk {entry['index']}: expected {size}, got {total_bytes}"
            )
